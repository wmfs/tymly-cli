...
async function newBlueprint (name, options) {
  ...
  const [usesOk, usesBlueprint] = checkUses(options.use, targetDirectory)
  if (!usesOk) {
    return
  }
  ...
  if (usesBlueprint) {
    blueprintInfo.meta.uses = [ usesBlueprint ]
  }
...
function checkUses (blueprintPath, targetDirectory) {
  if (!blueprintPath) {
    return [true, null]
  }

  const usesBlueprint = Blueprint.load({ path: blueprintPath })
  return [
    usesBlueprint !== null,
    path.relative(targetDirectory, blueprintPath)
  ]
}
